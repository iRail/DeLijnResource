...
//Initiate constants
    ...
    public static function LambertToWGS84($x, $y)
    {
        Tools::initvars();
        //calc
        $r = sqrt(($x - Tools::$x0) * ($x - Tools::$x0)
            + (Tools::$r0 - ($y - Tools::$y0)) * (Tools::$r0 - ($y - Tools::$y0)));
        ...
        for ($i = 0; $i < 20; $i++) { //20 ought to be enough for anyone?
            $phi = pi() / 2 - 2 * atan($t * pow((1 - Tools::$e * sin($phi))
                        / (1 + Tools::$e * sin($phi)), Tools::$e / 2));
        }
